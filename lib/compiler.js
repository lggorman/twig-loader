...
module.exports = function(id, tokens, pathToTwig) {
    ...
    if (includes.length > 0) {
        _.each(_.uniq(includes), function(file) {
            /**
             * Adding 'templates' here, in combination with 
             * alias: {'templates' : path.resolve(__dirname, 'templates')}
             * in the webpack config, allows us to include files 
             * without a relative or absolute path, e.g. `{% include 'nav.twig' %}`
             *
             */
            output.unshift("require("+ JSON.stringify('templates/' + file) +");\n");
        });
    }
